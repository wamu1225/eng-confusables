...
set out と set off はどちらも「出発する」の意味を持ちますが、set out は目的地に向けた具体的な着手（プロジェクトの開始等）にも使え、set off は物理的な出発や、連鎖反応（警報・議論）を引き起こす意味でよく使われます。set back は「進んでいたものを後方へ押し戻す」ことから遅延と、費用の意味（それにはいくらかかったか、を婉曲に表す口語表現）の両方を持ちます。set up は「土台の上に組み立てる」イメージで、組織の設立から機材の準備まで幅広く使われます。set aside は「対象を本筋の脇に置いて保管・後回しにする」イメージでお金や対立を取っておく意味を表し、set about は「対象の周囲に自分を置いて関わり始める」イメージで作業への着手を表します。`,
    previewPairs: ['set out vs set back（出発する・着手する／遅らせる）', 'set off vs set up（出発する・引き起こす／設立する・準備する）', 'set aside vs set about（取っておく・後回しにする／取り掛かる）'],
  },
  {
    id: 'make',
    verb: 'make',
...
make up for は「不足していた分を新たに作り出して埋め合わせる」イメージで、失われたものの補償を表します。make do with は「理想ではないが今あるもので何とかする」という妥協のニュアンスを持ち、make out は「ぼんやりしたものの輪郭を作り出して判別する」ことから、視覚的・意味的な理解の両方に使われます。make up は「バラバラなものをまとめ上げる」イメージから仲直り・作り話・構成という3つの意味に分かれ、make off with は「その場を離れる＋何かを伴う」の組み合わせで窃盗を表します。`,
    previewPairs: ['make up for vs make do with（補う／代用してすませる）', 'make out vs make for（理解する・判別する／向かって進む）', 'make up vs make off with（仲直りする・構成する／盗んで逃げる）'],
  },
  {
    id: 'pass',
    verb: 'pass',
...
break down は「構造そのものが崩れ落ちる」イメージで、機械の故障にも交渉の決裂にも使われます。break off は「つながっていたものを断ち切って切り離す」イメージで、婚約や外交関係のような継続していた関係の中断を表します。break out は「内部に閉じ込められていたものが外部へ突発的に噴き出す」イメージで、戦争・火事・発疹などの突然の発生や、監獄からの脱出に使われます。break up は「1つのまとまりがばらばらの破片に砕ける」イメージで、恋愛関係の終わりや集団の解散を表します。break away は「拘束や集団から離れた場所へ逃れる」イメージで組織からの離脱を表し、break through は「立ちはだかる壁を貫いて先へ進む」イメージで突破・発見を表します。`,
    previewPairs: ['break down vs break off（故障する・決裂する／中断する・打ち切る）', 'break out vs break up（突発的に発生する・脱出する／別れる・解散する）', 'break away vs break through（離脱する／突破する・発見する）'],
  },
  {
    id: 'come',
    verb: 'come',
