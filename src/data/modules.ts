...
📖 慣用句 **the law of the land**（国の法律・慣習）は、その国で広く通用しているルールを指すときによく使われます。**low** の複合語 **lowland**（低地）も、地形の話でよく登場します。形容詞 **low-key**（控えめな・目立たない）は、催しや対応が派手でなく落ち着いていることを表す語としてビジネスの場でもよく使われます。形容詞 **unlawful**（違法な）は law に否定の un- が付いた語で、lawful（合法な）の反対として使われます。`,
    keyPoints: [
...
📖 聖書に由来する **the prodigal son**（放蕩息子）は、家を出て放蕩の末に戻ってきた人を指す慣用的な表現として、ニュースや文学でも比喩的に使われます。似た音の語に **sun** の派生形容詞 **sunny**（日当たりのよい・陽気な）もあり、性格を形容する a sunny personality（明るい性格）という使い方もよく見られます。英語の姓には **Johnson（John の息子）・Robinson（Robin の息子）**のように、**son を末尾に付けて「〜の息子」を表す父称由来の名字**が数多く見られます。植物名 **sunflower**（ひまわり）も、太陽の方を向いて咲く花の性質から名付けられた sun の複合語です。`,
    keyPoints: [
      'son と sun は同音 /sʌn/',
...
📖 pear の仲間には **prickly pear**（ウチワサボテンの実）という植物名もあり、洋なしとは別の見た目のとげのある果実を指します。フィギュアスケートの種目 **pair skating**（ペアスケート）も、この pair（一対）から来た名称です。住み込みで家事や育児を手伝いながら語学を学ぶ制度を指す **au pair**（オーペア）も、フランス語で「対等に」を意味する表現に由来し、pair と同じ語根（ラテン語 par＝等しい）を共有しています。**scissors（はさみ）や trousers（ズボン）のように、2つの部分から成る道具・衣類は a pair of scissors のように「pair」を使って数えます**。この「対で1つ」という発想は pair の中心的な使い方のひとつです。クリスマスキャロル『The Twelve Days of Christmas』の一節 **"a partridge in a pear tree"**（梨の木にとまるヤマウズラ）でも、pear tree（洋なしの木）という語がよく知られています。`,
    keyPoints: [
...
📖 ロンドンには弁護士の養成・登録を担う伝統ある組織 **the Inns of Court**（法曹院）があり、Inn という語が正式な組織名にまで使われている例として知られています。ホテルへの到着・出発を表す **check in**（チェックインする）の in も、この前置詞・副詞の in と同じ語です。口語表現 **keep someone in the loop**（人に情報を伝え続ける・蚊帳の外にしない）も、この in を使った定番の言い回しです。句動詞 **give in**（屈する・降参する）も、抵抗をやめて相手の要求を受け入れるときに使う定番の表現です。形容詞 **indoor**（屋内の）も in を使った複合語で、indoor pool（屋内プール）のように屋外（outdoor）と対比して使われます。`,
    keyPoints: [
      'in と inn は同音 /ɪn/',
