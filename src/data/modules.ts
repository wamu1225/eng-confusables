...
📖 心理学の用語 **flow state**（フロー状態）は、作業に完全に没頭し集中しきっている心理状態を指し、心理学者チクセントミハイが提唱した概念として知られています。動詞 **overflow**（あふれる・(容量を)超える）は flow に over が付いた複合語で、川の氾濫やメールの受信箱がいっぱいになる状況など、幅広い場面で使われます。反対に **flawed**（欠陥のある）は flaw の形容詞形で、計画や論拠の弱点を指摘するときによく使われます。`,
    keyPoints: [
...
📖 brush には「（危険・法などと）**一瞬関わること・かすること**」という意味の名詞用法もあり、**a brush with death**（九死に一生を得る経験）、**a brush with the law**（法との軽い接触＝ちょっとした違反）という言い方で使われます。日用品では **toothbrush**（歯ブラシ）・**hairbrush**（ヘアブラシ）・**paintbrush**（絵筆）のように、用途を表す語と組み合わさる複合語が豊富です。blush の類語 **flush**（顔がほてる・紅潮する）も、興奮や暑さで顔が赤くなる場面で使われる近い意味の語です。`,
    keyPoints: [
...
📖 家計の話でよく出る慣用句 **make ends meet**（収支を合わせる・なんとかやりくりする）も、この meet を使った定型表現です。形容詞 **meaty**（肉厚な・食べ応えがある）は、比喩的に「**内容が濃い**」（a meaty report＝内容の濃い報告書）の意味でも使われます。古い英語では meat は「肉」に限らず「**食べ物全般**」を指しました。**sweetmeat**（甘い菓子・砂糖漬け果実）という語にその名残があり、現在の meat（肉）とは異なる古い意味で使われています。`,
    keyPoints: [
...
📖 形容詞 **soulful**（心のこもった・感情豊かな）は、歌声や表情など「魂が感じられる」ものを表現するときに使われます。イギリス英語のビジネス用語 **sole trader**（個人事業主）は、法人化せず一人で事業を営む人を指す正式な語として使われます。副詞 **solely**（もっぱら・単に）も、sole から派生した語として文章語でよく使われます。`,
    keyPoints: [
...
📖 pear の仲間には **prickly pear**（ウチワサボテンの実）という植物名もあり、洋なしとは別の見た目のとげのある果実を指します。フィギュアスケートの種目 **pair skating**（ペアスケート）も、この pair（一対）から来た名称です。住み込みで家事や育児を手伝いながら語学を学ぶ制度を指す **au pair**（オーペア）も、フランス語で「対等に」を意味する表現に由来し、pair と同じ語根（ラテン語 par＝等しい）を共有しています。`,
    keyPoints: [
...
📖 法律用語 **loophole**（抜け穴・法の抜け道）は hole を使った複合語です。形容詞 **wholesome**（健全な・健康的な）は whole から派生した語で、食べ物や娯楽が「体や心に良い」ことを表します。口語表現 **a hole-in-the-wall**（こぢんまりとした目立たない店）も、飲食店の紹介などでよく使われます。句動詞 **hole up**（（隠れて）閉じこもる）は、人目を避けてどこかに身を潜める様子を表す口語表現です。副詞 **wholly**（完全に・まったく）は whole の綴りに ly を足した語で、fully と近い意味で使われます。`,
    keyPoints: [
...
📖 恋愛の慰めの言葉としてよく使われる **"There are plenty more fish in the sea."**（海には他にもたくさん魚がいる＝出会いは他にもある）も、sea を使った有名な慣用句です。複合語 **seasick**（船酔いした）も、sea と結びついてよく使われる語です。動詞 **oversee**（監督する・取り仕切る）は see を語根に持ち、「上から見渡す」というイメージから「管理・監督する」の意味に転じた語です。`,
    keyPoints: [
      'sea と see は同音 /siː/',
...
📖 恋愛や理想の相手を語るときの決まり文句 **one and only**（ただ一人の・かけがえのない人）も、この数の one を使った表現です。形容詞 **one-sided**（一方的な・偏った）は、議論や試合が均衡を欠いている状況を表すときによく使われます。慣用句 **at one with** ~（〜と調和して・一体となって）は、心や行動がぴったり合っている状態を表す、ややフォーマルな言い回しです。won の名詞形はなく、勝利そのものを表すときは名詞 **a win**（勝利）を使います。**She got her first win.**（彼女は初勝利を挙げた）のように、動詞 win とは別に名詞としても使われます。`,
    keyPoints: [
      'one と won は同音 /wʌn/',
...
📖 which から派生した **whichever**（どちらでも・どれでも）は「選択肢のどれであっても構わない」という意味の複合関係詞です。witch の派生語 **witchcraft**（魔術・呪術）も物語や民間伝承でよく登場します。フォーマルな文章でよく使う **in which case**（その場合には）は、前の文の内容を受けて条件を付け加えるときの定型的なつなぎ表現です。witch の複合語 **witch doctor**（呪術医）は、伝統医療で祈祷や呪術を用いる治療者を指す語として人類学や物語で使われます。`,
    keyPoints: [
...
📖 ロンドンには弁護士の養成・登録を担う伝統ある組織 **the Inns of Court**（法曹院）があり、Inn という語が正式な組織名にまで使われている例として知られています。ホテルへの到着・出発を表す **check in**（チェックインする）の in も、この前置詞・副詞の in と同じ語です。口語表現 **keep someone in the loop**（人に情報を伝え続ける・蚊帳の外にしない）も、この in を使った定番の言い回しです。`,
    keyPoints: [
      'in と inn は同音 /ɪn/',
